...
def render_harvest_tab(harvest_df: Optional[pd.DataFrame]) -> None:
    ...
    if harvest_df is not None and not harvest_df.empty:
        ...
        # UX Fix: Relabel 'Avg Conv. Rate' to reflect its actual source
        if "Avg Conv. Rate" in harvest_df.columns and "Source Ad Group CVR" not in harvest_df.columns:
            harvest_df = harvest_df.rename(columns={"Avg Conv. Rate": "Source Ad Group CVR"})
