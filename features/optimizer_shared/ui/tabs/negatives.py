...
def render_negatives_tab(neg_kw: pd.DataFrame, neg_pt: pd.DataFrame, extract_validation_fn: Optional[Callable] = None) -> None:
    ...
    hidden_default_cols = {"CampaignId", "AdGroupId"}

    def _render_id_details(df: pd.DataFrame) -> None:
        id_cols = [c for c in ["CampaignId", "AdGroupId", "KeywordId", "TargetingId"] if c in df.columns]
        if not id_cols:
            return
